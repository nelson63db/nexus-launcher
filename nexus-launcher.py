...
# Cutoff for the git status probe on the project list, per repo.
GIT_STATUS_TIMEOUT = 2.0
...
def normalize_path(path):
    """Normalize a project path for comparison (~, escapes, trailing /)."""
    if not path:
        return ""
    return os.path.normpath(os.path.expanduser(path.replace('\\', '')))


def _probe_dirty(project):
    """True if the project is a git repo with uncommitted or untracked changes."""
    path = normalize_path(project.path)
    if not os.path.isdir(path):
        return project.alias, False
    try:
        ...
        return project.alias, False
    return project.alias, res.returncode == 0 and bool(res.stdout.strip())


def git_dirty_map(projects):
    """Probe every project at once; the menu redraws often and repos are slow."""
    if not projects:
        return {}
...
class TmuxOrchestrator:
    ...
    def launch_layout(self, project):
        ...
        subprocess.run(['tmux', 'rename-window', '-t', win, project.alias],
                       capture_output=True)
...
class NexusApp:
    ...
    def _display_projects_list(self, show_git=False):
        """Display formatted project list, optionally flagging dirty git repos."""
        if not self.manager.projects:
...
""")
    ...
    def main_menu(self):
        ...
        has_projects = self._display_projects_list(show_git=True)
...
""")
        ...
        if has_projects:
            print(f"  {C.DIM}Enter project number/alias to launch{C.RESET}")
        print()
  ...
  {C.YELLOW}PROJECT LAUNCHER{C.RESET}
    ...
    both list that directory's past sessions.
